class DropBoxController {
    constructor(){
        // Seletor do botão de envio de arquivos e o input fo tipo file para escolher arquivos.
        this.btnSendFileEl = document.getElementById('btn-send-file');
        this.inputFilesEl = document.getElementById('files');
        // Modal que informa o andamento do upload
        this.snackModalEl = document.getElementById('react-snackbar-root');
        this.progressBarEl = this.snackModalEl.querySelector('.mc-progress-bar-fg');
        this.nameFileEl = this.snackModalEl.querySelector('.filename');
        this.timeLeftEl = this.snackModalEl.querySelector('.timeleft');

        // Elementos carregados para preencher a tela com os arquivos armazenados no banco
        this.listFilesEl = document.querySelector('#list-of-files-and-directories');

        // Função que executa todas as ações para o envio de arquivos.
        this.initEvent();
    ...
    connectFirebase(){
        var firebaseConfig = {
            apiKey: "AIzaSyD1YNeRLF89rJ7fgCcvisKQky_QA8cnZQA",
            authDomain: "dropbox-clone-66c37.firebaseapp.com",
            databaseURL: "https://dropbox-clone-66c37-default-rtdb.firebaseio.com",
            projectId: "dropbox-clone-66c37",
            storageBucket: "dropbox-clone-66c37.appspot.com",
            messagingSenderId: "971868079483",
            appId: "1:971868079483:web:22ca90778cfa6122e929de",
            measurementId: "G-VZYKZ1DHV6"
          };
          // Initialize Firebase
    ...
    // Evento click para o botão de envio de arquivos.
    initEvent(){
    ...
    // fecha se o modal estiver aparecendo na tela e vice-versa
    modalShow(show = true){
        this.snackModalEl.style.display = (show) ? 'block':'none';
    }

    uploadTask(files){//Método que receberá os Files selecionados para upload. É uma coleção de arquivos, precisa ser convertido em array usando o spred => [...]
        let promises = [];// Array pois pode receber vários arquivos, deve ser inserido o atributo multiple no campo input do   tipo FILE.
        ...
        [...files].forEach(file=>{
            promises.push(new Promise((resolve, reject)=>{
                ...
                //monitoramento das atividades do ajax
                ajax.onload = event => {
                    try {
    ...
    // Carrega a barra de progresso do upload.
    uploadProgress(event, file){
        let timespent = Date.now() - this.startUploadTime;
        let loaded = event.loaded;
        let total = event.total;  
        let porcent = parseInt((loaded/total) * 100);//calculo de tamanho em tempo real do upload.
    ...
    // calcula a duração do upload e formata para hora, minuto ou segundo.
